...
BIDS=$PWD/../Data/bids-3depi/
sub=sub-1
run=''
# ../Data/bids-3depi/sub-1/fmap/sub-1_acq-largefov_magnitude1.json
# ../Data/bids-3depi/sub-1/func/sub-1_task-n40p20_acq-inc_bold.nii.gz

epi="$BIDS/$sub/func/${sub}_task-n40p20_acq-inc${run}_bold.nii.gz"
mag="$BIDS/$sub/fmap/${sub}_acq-largefov_magnitude1.nii.gz"
phase="$BIDS/$sub/fmap/${sub}_acq-largefov_phasediff.nii.gz"

out=$PWD/wf/human-largefov-3depi/$sub$run
mkdir -p $out
