...
mtbc_p <- cor_for_shapes |>
  ...
  #ggforce::geom_circle() +
  geom_text(aes(size=text_size, color=sign, alpha=abs(value)), data=cor_for_text, vjust=0, hjust=0) +
  ggforce::geom_mark_rect(aes(group=annote, label=NULL), color='black', size=1, fill=NULL, expand=.1, position=position_nudge(x=.15,y=.1), alpha=.8,
...
#+begin_src R :session :width 11.5 :height 10 :res 300 :units in :results graphics file :file Figures/orgmode/inflow_2Dto3Dsimulated.png
...
#+RESULTS:
[[file:Figures/orgmode/inflow_2Dto3Dsimulated.png]]
...
explode_4d_df <-function(img, sess='1', measure='tsnr')
  lapply(seq_along(msa_angles5),
                         \(i) data.frame(tsnr=as.vector(img[,,,i]),
                                         angle=msa_angles5[i],
                                         sess='1',
...
tsnr4d_2 <- "AngleCompare/maxangle_mni/sub-2_tsnr-n40n33n13p13p20.nii.gz"
msa4d_1 <- "./maxangle_mni/sub-1_task-n40p20_acq-incSUBSET_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz"
msa4d_2 <- "./maxangle_mni/sub-2_task-n40p20_acq-incSUBSET_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz"

tsnr_vecs_roi <- rbind(
    explode_4d_df(tsnr_a10,sess='1',measure='tsnr'),
    explode_4d_df(read_and_crop(tsnr4d_2), sess='2',measure='tsnr'),
...
tsnr_smry <- tsnr_vecs_roi |>
    ...
    summarise(stat=median(tsnr),
              diff=diff(range(tsnr))) |>
    ungroup()|>
...
p_tsnr_bar <- ggplot(tsnr_smry) +
  aes(x=angle,
      ...
      fill=measure,
      color=is_max) +
  geom_bar(stat='identity') + #stat='summary', fun='median') + # \(x) (mean(x)-min(x))/mean(x)) +
  #ggbeeswarm::geom_beeswarm(data=tsnr_vecs_roi|>filter(tsnr>0), aes(y=tsnr,color=NULL), cex=.2,alpha=.4) +
    #gghalves::geom_half_violin(data=tsnr_vecs_roi|>filter(tsnr>0)|>mutate(is_max=F), aes(y=tsnr), alpha=.4) +
  facet_grid(sess~roi) +
  scale_color_manual(values=c("#ffffff00","#000000"), guide="none") +
  theme_bw() +
  theme(panel.grid=element_blank()) +
  labs(x='ɸ', y='tSNR', color='Measure')
print(p_tsnr_bar)
#+end_src


#+begin_src R :session
# for picking location wiht afni
...
delta_tsnr_brain <- ggbrain(bg_color="white", text="black") +
    ...
    geom_brain("tsnr", alpha=.5,
               fill_scale = scale_fill_viridis_c("ΔtSRN", option="magma"),
               show_legend=T)+
...
delta_tsnr_brain + slices(c("x=12")) +
    render() +
    labs(title="range of tSNR")
 #+end_src
...
#+begin_src R :session :results graphics file :file Figures/orgmode/tsnr_B0/scatter_angle_color.png
...
#+begin_src R :session :results graphics file :file Figures/orgmode/tsnr_B0/scatter_magVdiff.png
...
ggplot(sub1_tbd|>filter(minmaxdiff>0,b0_large))+
    ...
    facet_grid(.~factor(sign(b0_1),levels=c(-1,1), labels=c("-B0","+B0")), scales='free') +
...
#+begin_src R :session :results graphics file :file Figures/orgmode/tsnr_B0/histogram.png
ggplot(sub1_tbd|>filter(minmaxdiff>0, b0_large))+
    aes(x=b0_1, fill=tsnr_1) +
    geom_histogram(position='dodge', bins=5) +
    theme_bw() +
    scale_fill_manual( values=angle_colors_div) +
    facet_grid(cut(minmaxdiff,quantile(minmaxdiff, c(0, .4,.8,1))+1)~sign(b0_1),scales='free')
...
|------+------+-----------+--------+----------+--------|
| AON  |    1 |       -21 |     39 |      -24 |     62 |
| AON  |  3d1 |       -14 |    177 |      -29 |     90 |
| PirF |    1 |       -14 |     10 |      -32 |     10 |
| PirF |  3d1 |        -7 |     10 |      -24 |     10 |
| PirT |    1 |       -13 |    216 |      -18 |    144 |
| PirT |  3d1 |       -13 |    142 |      -20 |    170 |
| Tub  |    1 |        -9 |     12 |      -22 |     21 |
| Tub  |  3d1 |        -8 |     13 |      -14 |    153 |
| V4   |    1 |        -1 |     18 |       -2 |     45 |
| V4   |  3d1 |        -3 |     20 |       10 |     18 |
|      |    1 |        -9 | 368638 |      -20 | 368157 |
|      |  3d1 |        -8 | 367526 |      -14 | 368156 |
