...
#+begin_src bash
3dinfo -n4 FmapCorrect/wf/human-largefov/sub-1/sub-1_acq-largefov_magnitude1.nii.gz 
#+end_src
...
see [[file:Figures/MSA_max.svg]] which uses [[file:Figures/montage_nifit.R]] for nifti slice images.
  ...
  * 20260509 - added actual MSA shot sub1
...
** any ROI where tSNR is worse at +20? (V4)
...
** B0 against max tsnr all voxels
...
# CANT USE -- invalid tsnr: no mean in bandpassed
...
cor_for_text <- msa_tsnr_b0_cor_r |>
  melt_cor() |>
  mutate(sign=as.factor(sign(value)),
         text_size=abs(value)/2 ,
         #annote = grepl('tsnr',paste0(Var1,Var2)) & grepl('msa',paste0(Var1,Var2)))
         annote = paste0(Var1,Var2) %in% c('tsnr_1msa_1','tsnr_2msa_2', 'b0_1tsnr_1','b0_2tsnr_2', 'b0_1msa_1','b0_2msa_2'))

#msa_cor_annotations <- function(Var1s='tsnr_1',Var2s='msa_1')
#  ggforce::geom_mark_rect(aes(group=annote, label=NULL),
#                          color='black', size=1, fill=NULL,
#                          expand=.1,
...
mtbc_p <-
  cor_for_shapes |>
  ggplot()+
  aes(y=Var1,x=Var2, fill=round(value,2),
      size=abs(value), label=round(value,2)) +
  geom_point(shape=21,stroke=0) +
  scale_radius(guide="none", range=c(3,18), limits=c(0,1)) +
  #ggforce::geom_circle() +
  geom_text(size=7, aes(size=NULL,color=sign, alpha=abs(value)), data=cor_for_text, vjust=0, hjust=0) + # size=text_size,
  geom_point(data=cor_for_text|>filter(annote,!is.na(value)),
             aes(color=NULL),
             position=position_nudge(x=.5, y=-.1),
             shape=8,
             size=1,
             color='black') +
  #msa_cor_annotations('tsnr_1','msa_1') +
  ...
  theme(panel.grid=element_blank(),
        ...
        axis.text.x=element_text(hjust=-.5, size=15, angle=-45, vjust=0, color=c(acq_colors$msa,acq_colors$msa,acq_colors$tsnr,acq_colors$tsnr,'black','black')),
        axis.text.y=element_text(hjust=2.2, size=15, angle=45, vjust=-1, color=rev(c(acq_colors$msa,acq_colors$msa,acq_colors$tsnr,acq_colors$tsnr,'black','black'))))
...
*** How often is MSA and tSNA picking the same?
What angle is MSA off by? how big is the tSNR difference between picked and actual.

#+begin_src R :session :results graphics file :file Figures/orgmode/tsnr_msa_assignments_hist.png :width 6 :height 4 :res 300 :units in
msatsnr_matching_i <- all_vecs$msa_1 == all_vecs$tsnr_1
length(which(msatsnr_matching_i)) / length(msatsnr_matching_i) * 100 # 8%

# NOT USED. gird of mismatches
p_msatsnr_matches <- all_vecs |>
  select(msa_1,tsnr_1)|>
  filter(!is.na(tsnr_1),tsnr_1!=0,msa_1!=0) |>
  mutate(across(c(msa_1,tsnr_1), as.factor)) |>
  count(msa_1,tsnr_1) |>
  group_by(tsnr_1) |>
  mutate(of_tSNR = n/sum(n)) |>
  ggplot() +
  aes(x=msa_1,y=tsnr_1, fill=of_tSNR) +
  geom_tile() +
  geom_text(aes(label=round(of_tSNR,2))) +
  labs(fill="Portion\nof tSNR",
       y=parse(text="ɸ[tSNR]"),
       x=parse(text="ɸ[MSA]")) +
  see::theme_modern() +
...
p_msatsnr_hist <- with(all_vecs, rbind(data.frame(x=msa_1,meas='MSA'), data.frame(x=tsnr_1,meas='tSNR'))) |>
  filter(!is.na(x), x!=0) |>
  mutate(x=as.factor(x)) |>
  ggplot() +
  aes(x=x, fill=meas) +
  geom_histogram(position='dodge',stat='count') +
  labs(y="n Vox.", fill="", x="ɸ" )+
  see::theme_modern() +
...
all_vecs |>
  filter(msa_1!=0, tsnr_1!=0, !is.na(msa_1)) |>
  #mutate(msa_1=sprintf("<span style='color: %s'>%s</span>",angle_colors_div[as.character(msa_1)],  msa_1)) |>
ggplot() +
  aes(x=as.factor(msa_1), group=tsnr_1, fill=as.factor(tsnr_1)) +
  geom_histogram(stat='count', position='dodge') +
  labs(y='n Vox.', fill=parse(text='ɸ[tSNR]'), x=parse(text="ɸ[MSA]")) +
  scale_fill_manual(values=angle_colors_div) +
  see::theme_modern() +
  theme(
    axis.text=element_text(size=12),
    #axis.text.x=ggtext::element_markdown(angle=-90, hjust=0, vjust=0.5),
    axis.text.x=element_text(angle=-90, hjust=0, vjust=0.5),
    axis.title=element_text(size=18))

p_msatsnr_hist_combo <- (p_msatsnr_hist  / p_mismatch_hist) +plot_layout(guides="collect", axis="collect")


#+end_src

#+RESULTS:
[[file:Figures/orgmode/tsnr_msa_assignments_hist.png]]
...
(p_brain_msa_1  | p_brain_tsnr_1) / ( (p_avsl+guides(fill="none")) | mtbc_p+guides(fill="none"))
#+end_src
#+RESULTS:
[[file:Figures/orgmode/maxangle/msa_vs_tsnr.png]]

#+begin_src R :session :results graphics file :file Figures/orgmode/maxangle/msa_vs_tsnr_hist.png :session :width 11.5 :height 10 :units in :res 300
# bottom right is breakdown of asignments
(p_brain_msa_1  | p_brain_tsnr_1) / (( p_avsl + guides(fill="none")) | p_msatsnr_hist_combo)
#+end_src
...
corplot_as_ggplot <- function(x,text_size=10) {
  x <- phi_to_int_col(x)
  mat_l <- round(x,2);
  mat_r <- mat_l
  ...
  ggplot(mat_shapes)+
    ...
    geom_text(aes(size=NULL, #abs(value)/2,
...
colnames(tsnr_a10_colwise) <- paste0(msa_angles5,'_2D')


epi3d2d_cor <- 1:dim(tsnr)[4] |>
    sapply( \(i) cor(as.vector(tsnr[,,,i]), tsnr_a5_colwise,
                 use='pairwise.complete.obs'))
colnames(epi3d2d_cor) <- paste0(msa_angles5,'_2D')
rownames(epi3d2d_cor) <- paste0(msa_angles5,'_3D')

# confirm order
epi3d2d_cor_one <- cor(as.vector(tsnr[,,,1]), tsnr_a5_colwise, use='pairwise.complete.obs')
stopifnot(all(epi3d2d_cor_one == epi3d2d_cor[,"-40_2D"]))
...
** p_epi3d2d_cor: Plot 3depi vs 2depi like other correlation matrices

#+begin_src R :session :results graphics file :file Figures/orgmode/tsnr_3depi_2depi_full_corrmat.png :width 7 :height 6
 axis_colors_msa5 <- angle_colors_div[msa_angles %in% msa_angles5]
 p_epi3d2d_cor <-
  epi3d2d_cor|> round(2) |> reshape2::melt() |>
  ggplot()+
    aes(y=Var1,x=Var2, color=value, label=value) +
    #geom_point(shape=21,stroke=0) +
    #scale_radius(guide="none", range=c(3,10), limits=c(0,1)) +
    geom_text(size=10)+
    theme_minimal() +
    ...
    theme(panel.grid=element_blank(),
        plot.title=element_text(hjust=.5,size=20),
        axis.text.x=element_text(size=15, hjust=0, vjust=1, angle=-45,
                                 color=axis_colors_msa5),
        axis.text.y=element_text(size=15, hjust=0, vjust=1 ,angle=45,
                                 color=axis_colors_msa5))+
    labs(fill="r", y="",x="", title="Voxelwise tSNR 3D~2D")
 print(p_epi3d2d_cor)
#+end_src

#+RESULTS:
[[file:Figures/orgmode/tsnr_3depi_2depi_full_corrmat.png]]
...
**** combo plot with B0 threshold
#+begin_src R :session

p_msatsnr_hist_b0 <- with(all_vecs|>filter(abs(b0_1)>200),
                          rbind(data.frame(x=msa_1,meas='MSA'),
                                data.frame(x=tsnr_1,meas='tSNR'))) |>
  filter(!is.na(x), x!=0) |>
  mutate(x=as.factor(x)) |>
  ggplot() +
  aes(x=x, fill=meas) +
  geom_histogram(position='dodge',stat='count') +
  labs(y="n Voxs", fill="", x="ɸ" )+
  theme_minimal() +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=18))

p_mismatch_hist_b0 <-
all_vecs |> filter(abs(b0_1)>200, msa_1!=0, tsnr_1!=0, !is.na(msa_1)) |>
ggplot() +
  aes(x=as.factor(msa_1), group=tsnr_1, fill=as.factor(tsnr_1)) +
  geom_histogram(stat='count', position='dodge') +
  labs(y='n Vox.', fill='ɸ[tSNR]', x=parse(text="ɸ[MSA]")) +
  scale_fill_manual(values=angle_colors_div) +
  theme_minimal() +
  theme(
    axis.text=element_text(size=12),
...
(p_msatsnr_hist_b0  / p_mismatch_hist_b0) +plot_layout(guides="collect")

(p_msatsnr_hist  / p_mismatch_hist) +plot_layout(guides="collect")
#+end_src
    ...
    data.frame(cor =
                   sapply(seq_along(tsnr_caps), \(i)
                          cor(all_vecs$tsnr_1[tsnr_1a5d>=tsnr_caps[i]],
                              ...
                              use='pairwise.complete.obs')),
               nmatch=sapply(seq_along(tsnr_caps), \(i)
                          length(which(all_vecs$tsnr_1[tsnr_1a5d>=tsnr_caps[i]]==all_vecs$msa_1[tsnr_1a5d>=tsnr_caps[i]]))),
    ...
    data.frame(cor =
                   sapply(seq_along(tsnr_caps), \(i)
                          cor(all_vecs$tsnr_2[tsnr_2a5d>=tsnr2_caps[i]],
                              ...
                              use='pairwise.complete.obs')),
               nmatch=sapply(seq_along(tsnr_caps), \(i)
                          length(which(all_vecs$tsnr_2[tsnr_2a5d>=tsnr2_caps[i]]==all_vecs$msa_2[tsnr_2a5d>=tsnr2_caps[i]]))),
               n = sapply(seq_along(tsnr_caps), \(i)
                          length(which(!is.na(all_vecs$tsnr_2[tsnr_2a5d>=tsnr2_caps[i]])))),
               thres=tsnr2_caps,
               quant= tsnr_caps_quant,
               sess="sub-2"))

tcor_max <- tsnr_cor_by_quant |> filter(quant<.8) |>group_by(sess) |>  filter(cor==max(cor,na.rm=T))
print.data.frame(tcor_max)
 #+end_src

 #+RESULTS:
 |               cor | nmatch |      n |            thres | quant | sess  |
 |-------------------+--------+--------+------------------+-------+-------|
 | 0.198641685837161 |  20147 |  93367 | 13.3697052001953 |   0.6 | sub-1 |
 |  0.15648496281894 |  32232 | 163643 | 7.55513000488281 |   0.3 | sub-2 |
...
p_tsnr_cor_quant <-  ggplot(tsnr_cor_by_quant) +
      aes(x=quant, y=cor,
          #y=nmatch/n,
          size=n, color=gsub('sub-','',sess)) +
      geom_hline(data=tcor_max, aes(yintercept=cor), color="gray") +
      geom_line(data=tsnr_cor_by_quant|>filter(quant<.99), aes(y=nmatch/n, size=NULL,group=sess),alpha=.4, size=1) +
      geom_point() +
      see::theme_modern() +
...
*** B0 distibrution and quantile threshold
Also see ./Figures/b0_thres_animate.R
...
# to pair with b0_signed_minmax
b0_diff_by_angle <- ggplot(sub1_tbd|>filter(minmaxdiff>0))+
...
b0_signed_minmax <- ggplot(sub1_tbd|>filter(minmaxdiff>0,b0_large))+
    aes(x=b0_1,
        y=minmaxdiff,
    ...
    facet_grid(.~factor(sign(b0_1),levels=c(-1,1), labels=c("< 0"," > 0")), scales='free') +
...
*** Combine
pair to output of ./Figures/b0_thres_animate.R output Figures/b0/brain_cor_qaunt.gif

#+begin_src R :session :results graphics file :file Figures/orgmode/tsnr_B0/scatter_angle_color_and_MinMax.png :width 10 :height 6 :res 300 :units in
(b0_diff_by_angle / b0_signed_minmax) + plot_layout(guides='collect',axis='collect')
#+end_src

#+RESULTS:
[[file:Figures/orgmode/tsnr_B0/scatter_angle_color_and_MinMax.png]]
...
msa_tsnr_by_b0_quant <- rbind(
 ...
 msa_tsnr_b0_quant_roi <- msa_tsnr_by_b0_quant |>
  transmute(cor, sess, quant, n, across(c(nV4,nAON,nTub), \(x) x/max(x))) |>
  gather(roi,percent,-c(cor,sess,quant,n))

p_b0_cor_quant <-  ggplot(msa_tsnr_by_b0_quant) +
      aes(x=quant, y=cor, size=n, fill=gsub('sub-','',sess)) +
      #geom_hline(data=tcor_max, aes(yintercept=cor), color="gray") +
...
p_iso3d_cor <-
  corplot_as_ggplot(iso3d_cmp_mat, text_size=5) +
  # why do we need to switch these?
  scale_color_manual(values=c('blue','red'), guide="none") +
  scale_x_discrete(labels=relabel_datasets_axis) +
  scale_y_discrete(labels=relabel_datasets_axis) +
  ggforce::geom_mark_rect(aes(group=annote, label=NULL),
                          color='black', size=1, fill=NULL, expand=.05, position=position_nudge(x=.25,y=.08),
  ...
  theme(plot.title=element_text(hjust=.5,size=20),
        ...
        axis.text.y=element_text(hjust=.5,size=15))
print(p_iso3d_cor)
#+end_src
...
*** MSA tSNR argmax on the Brain
combine MSA tSNR and tSNR 2D
...
#+RESULTS:
[[file:Figures/orgmode/maxangle/3diso_msa_tsnr_roi_brain.png]]

*** TODO Where is there mismatch?
HERE HERE
see delta_tsnr_brain
MSA - tSNR ?
low delta B0  line plot?

*** Combined
2d vs 3d tSNR brain, roi, and cormap ([[*p_epi3d2d_cor]])

#+begin_src R :session :results graphics file :file Figures/orgmode/3diso_MSAs_vs_tsnr.png :session :width 12 :height 8 :res 300

pbt_1 <- p_brain_tsnr_1 +  ggtitle("tSNR 2D") + theme(plot.title=element_text(color="black"))


# large cormap of everything
...
# 2d to 3d epi
p_2d_v_3d_grid <-
    (p_brain_msa_3d1  / p_brain_tsnr_3d1 /pbt_1 ) |
    ( (p_iso_roicnt+guides(fill="none")) /
      (p_epi3d2d_cor +
       guides(color="none") +
...
* Log                                                            :noexport:
** 2026-05-09
 * add Figures/FoV.png Figures/distortion_by_angle.gif
 * replace Figures/tSNR_angle.gif
 * replace MSA [[file:Figures/orgmode/maxangle/msa_vs_tsnr.png]]
    - maybe regenerate after installing ggtext

** 2026-05-04
*** initial figures implemented
*** TODO results
**** sub-1 vs sub-2 tSNR vs MSA
*** DONE regenerate intro figure
**** add ROIs
**** citation/justification for AON
*** DONE regenerate animation
